...
**Published image (this fork, 2.7.7):** `dx616b/spoti-to-navidrome:2.7.7` (also tagged `:latest` on Docker Hub).
...
docker run -d -p 8000:30321 --name downtify \
  ...
  dx616b/spoti-to-navidrome:2.7.7
```
...
See [`docker-compose.example.yml`](docker-compose.example.yml) for a ready-made stack using `dx616b/spoti-to-navidrome:2.7.7`.
...
services:
  downtify:
    container_name: downtify
    image: dx616b/spoti-to-navidrome:2.7.7
    ports:
      - '8000:30321'
    environment:
      - DOWNTIFY_PORT=30321
