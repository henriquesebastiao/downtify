...
On the Top Songs page, and on the page a pasted **album** or **playlist** link opens, any song that isn't downloaded yet can be previewed: hover its row and the number turns into a play button (on a phone the play button is always there); press it to hear a 30-second clip right on the row, with a ring around the button that fills as the clip goes. Press it again to pause, or another row's button to switch. A song that is already in your library plays through the built-in player instead, with the list's other downloaded songs queued after it. Like on the Library's [Top songs tab](#on-an-artists-library-page), clips never go through the built-in player and are never saved.

Where the clip comes from:

- **Spotify** songs bring their own clip — for a long playlist, the first 50–100 or so tracks (what Spotify's embed page lists).
- **Anything else** — YouTube Music songs, and Spotify playlist tracks past that point — is looked up on **Deezer** the first time you press play: a song by the same artist with the same title (ignoring `(Live)`, `[Remastered]` and `- Radio Edit` style suffixes), the one closest in length. A song Deezer has nothing for says so and stops offering a preview.
...
An artist Downtify knows on Spotify (see [Artist photo, banner & bio](artist-images.md)) also gets a **Top songs** tab on their page in your Library, next to Tracks and Fans also like. It lists the first **5** of their Spotify top songs as a minimal list, like the search results, under column headings (#, Title, Album, length): number, cover, title, artist, album and length. A song that isn't in your library has a **download** button on the right (with its progress while it downloads); once it's downloaded that turns into the **In library** label, and the song can be played the way it is on an album page: the number turns into a play button on hover, the cover plays, and a double click or a tap plays too. Playing one queues the list's other downloaded songs after it, in ranking order, and the song being played is highlighted. A song that isn't downloaded yet can be **previewed** instead: hover its row, and where the number is a play button appears (and the song's length on the right reads **Preview**, in your language, while the mouse is over the row and for as long as its clip is loaded); press it (or the cover) to hear the 30-second clip Spotify offers for it, right on its row. The number is replaced by a pause button inside a ring that fills as the clip goes, and the title is highlighted; press it again to pause, or another song's play button to switch. It doesn't use the built-in player: starting a clip pauses the player, and starting the player (or downloading that song and playing it) ends the clip. The clip is streamed straight from Spotify at the player's volume and never saved; for a song Spotify has no clip for, Downtify looks one up on Deezer instead (see [Previewing songs before downloading](#previewing-songs-before-downloading)). Below the list, **See more popular songs** opens this artist on the Top Songs page described above, which is where the selection, the **Create playlist** switch and downloading several at once live. The tab only appears once the artist's Spotify id is known, which happens the first time their page is opened; the songs are asked for right then, so the tab's count fills in by itself a few seconds later, wherever you are on the page.

Reading a song's cover, album and play count takes a few seconds, so Downtify keeps the five songs in a file per artist, `<downloads>/.metadata/ArtistTopSongs/<Artist>.topsongs.json`, made in the background the first time the artist's page is opened. It is trusted for **7 days**: opening the tab reads the file at once, and once it is older than that the old list is shown right away while a fresh one is fetched behind it - the page swaps it in by itself within a few seconds, without disturbing a song that is downloading or playing. The file only holds the songs, their cover and preview-clip links and play counts - never the audio or the images, and never whether a song is in your library, which is worked out live. A file saved before the previews existed counts as old at once, so the clips show up the next time the tab is opened. It's a cache: delete it whenever you like and it is made again.
