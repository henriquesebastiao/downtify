...
Downtify checks its own [GitHub Releases page](https://github.com/henriquesebastiao/downtify/releases) once an hour. When a newer version is out, a small notice appears centered in the page footer — on every page, since the footer is shared across the whole app:

> ⬇ Version 2.12.0 is available
...
- The web UI reads the cached result from `GET /api/check_update` (see [API Reference](../api-reference.md)) once per hour and shows the footer notice when a newer version is found.
